...
mips_trap(struct trapframe *tf)
{
      ...
      /* Syscall? Call the syscall handler and return. */
        ...
        switch (code) {
        case EX_MOD:
                if (vm_fault(VM_FAULT_READONLY, tf->tf_vaddr)==0) {                     /*questo errore viene generato se c'è un tentativo di scrivere sulla sezione di codice ("text") che è presente nella TLB e che deve essere impostato a priori come sola lettura*/
                        goto done;
                }
                break;
        case EX_TLBL:            /*MISS*/                                               /*questo errore si verifica se manca la pagina cercata*/
                if (vm_fault(VM_FAULT_READ, tf->tf_vaddr)==0) {
                        goto done;
                }
                break;
        case EX_TLBS:                                                                   /*questo errore DOVREBBE verificarsi quando OS tenta di scrivere ma non c'è spazio*/
